...
"""SQL-only access to PlaylistSongEntity (PK id — synthesized by
PlaylistSongMapper as "{playlistId}:{songId}"). Never sees HTTP.
...
_COLUMNS = ("id", "songId", "playlistId", "position", "multiUserId")

_UPSERT_SQL = "INSERT OR REPLACE INTO PlaylistSongEntity ({}) VALUES ({})".format(
    ", ".join(_COLUMNS),
    ", ".join("?" * len(_COLUMNS)),
)
...
class PlaylistSongRepository:
    ...
    def upsertPlaylistSongs(self, rows) -> None:
